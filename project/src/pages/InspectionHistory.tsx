...
const InspectionHistory: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAllPanelHistories = async () => {
          ...
          panels.map(async (panel) => {
            try {
              const response = await axiosInstance.get(
                `/api/user/panels/${encodeURIComponent(panel.id)}/history`,
              );
              const history = response.data || [];

              history.forEach((inspection: any) => {
                inspections.push({
                  ...inspection,
                  panelId: panel.id,
              ...
              console.error("Error fetching history for panel", panel.id, err);
            }
          }),
        );
  ...
  const handleFilterChange = (level: string | null) => {
    ...
    if (level === null) {
      ...
      setFilteredInspections(
        allInspections.filter((inspection) => inspection.faultLevel === level),
      );
    }
  };
              ...
              <div key={index} className="py-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-2">
                  <div className="flex items-center mb-2 md:mb-0">
                    ...
                    <Link
                      to={`/panel/${inspection.panelId}`}
                      className="font-medium text-blue-600 hover:text-blue-800"
                    >
                      {inspection.panelId}
                    </Link>
                    <span className="text-gray-500 text-sm ml-2">
                      (Row {inspection.panelPosition.row}, Col{" "}
                      {inspection.panelPosition.column})
                    </span>
                  </div>

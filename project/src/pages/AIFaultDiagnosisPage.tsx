...
Format the response in clear sections with headings.`;
      ...
      const diagnosisText = response.data?.candidates?.[0]?.content?.parts?.[0]?.text || 
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-3xl shadow-xl p-8 mb-8">
          <h1 className="text-4xl font-bold text-orange-700 mb-6">AI Fault Diagnosis Assistant</h1>
          <p className="text-gray-600 mb-8">
            Select a panel to get detailed fault analysis, repair procedures, and maintenance recommendations.
          </p>

          {/* Panel Selection */}
          <div className="mb-8">
            <label className="block text-gray-700 font-semibold mb-2">Select Panel</label>
            {/* Panel Search Bar */}
            <input
              type="text"
            ...
            >
              ...
              {panels
                .filter(panel =>
                  panel.id.toLowerCase().includes(panelSearch.toLowerCase()) ||
                  (panel.currentFault.description && panel.currentFault.description.toLowerCase().includes(panelSearch.toLowerCase()))
                )
                .map(panel => (
                  <option
                    key={panel.id}
                    ...
                    style={{ color: '#1a202c', backgroundColor: '#fff' }}
                  >
                    {panel.id} - {panel.currentFault.description} ({panel.currentFault.level})
                  </option>
                ))}
            </select>
          </div>
              ...
              <div className="mt-8 p-6 bg-orange-50 rounded-xl border border-orange-200">
                ...
                <div className="space-y-6">
                  {sections.map((section, idx) => {
                    ...
                    return (
                      <div key={idx} className="bg-white rounded-xl shadow p-4 border-l-4 border-orange-300">
                        {heading && (

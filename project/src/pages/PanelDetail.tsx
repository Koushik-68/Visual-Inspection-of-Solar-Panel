...
import { Download, ArrowLeft, AlertTriangle, Info, Sun, History, Wrench, Calendar, MapPin, Maximize } from 'lucide-react';
...
import PanelRegistrationForm, { PanelData } from '../components/PanelRegistrationForm';
import {
...
const PanelDetail: React.FC = () => {
  ...
  const getPriorityFromDescription = (description: string) => {
    ...
    if (desc.includes('dust') || desc.includes('bird') || desc.includes('snow')) return 'medium';
  ...
  if (panel && panel.currentFault) {
    ...
    faultLevelColor = mappedPriority === 'high' ? '#ef4444' : 
                     mappedPriority === 'medium' ? '#f59e0b' : 
                     mappedPriority === 'low' ? '#fbbf24' : '#10b981';
    faultDescription = panel.currentFault.description;
  }

  // Set maintenance suggestion based on mapped priority
  let maintenanceSuggestion = (panel && panel.maintenanceSuggestion) ? panel.maintenanceSuggestion : '';
  if (mappedPriority === 'low') maintenanceSuggestion = 'No immediate action required';
  else if (mappedPriority === 'medium') maintenanceSuggestion = 'Regular cleaning recommended';
  else if (mappedPriority === 'high') maintenanceSuggestion = 'Repair/Replacement needed';
  ...
  if (!panel) {
    return (
      <div className="text-center py-12 animate-fadeIn">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Panel Not Found</h2>
        <p className="text-gray-600 mb-6">The panel you're looking for doesn't exist or has been removed.</p>
        <Button 
          variant="primary" 
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 shadow-lg -m-6 mb-6 p-6 text-white">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap gap-4 mt-6">
            <div className={`flex items-center space-x-2 rounded-full px-4 py-2 animate-fadeIn ${getFaultLevelBgClass(mappedPriority)}`} style={{animationDelay: "0.3s"}}>
              <AlertTriangle className="h-4 w-4 text-white" />
              <span>Status: <span className="font-semibold capitalize">{mappedPriority === 'none' ? 'Healthy' : `${mappedPriority} priority`}</span></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <Card title="Panel Information" className="overflow-hidden transition-all duration-300 hover:shadow-md animate-slideInLeft">
              {editMode ? (
                ...
                <>
                  ...
                  <div className="flex justify-center mb-4">
                    <span className={`inline-block px-6 py-2 rounded-full font-semibold text-lg shadow-md border-2 transition-all duration-300
                      ${
                        mappedPriority === 'high' ? 'bg-red-100 text-red-700 border-red-400' :
                        mappedPriority === 'medium' ? 'bg-yellow-100 text-yellow-800 border-yellow-400' :
                        mappedPriority === 'low' ? 'bg-green-100 text-green-700 border-green-400' :
                        'bg-blue-100 text-blue-700 border-blue-400'
                      }
                    `}>
                      Priority: <span className="capitalize">{mappedPriority === 'none' ? 'Healthy' : `${mappedPriority} priority`}</span>
                    </span>
                  </div>
                    ...
                    <div className="mb-4 flex flex-col items-center">
                      <div className="w-80 h-64 bg-gray-200 rounded-lg shadow-inner border-2 border-amber-400 flex items-center justify-center relative mb-2">
                        <img
                          ...
                          style={{ background: 'linear-gradient(135deg, #fcd34d33 0%, #fbbf2433 100%)' }}
                        />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-3 rounded-lg border border-gray-100 bg-white transition-all duration-300 hover:bg-gray-50">
                  <div className="flex items-center mb-2">
                    <Info className="h-4 w-4 text-blue-500 mr-2" />
                    <p className="text-sm font-medium text-gray-600">Panel ID</p>
                  </div>
                      <p className="text-base text-gray-900 pl-6">{panel?.id || ''}</p>
                </div>
                <div className="p-3 rounded-lg border border-gray-100 bg-white transition-all duration-300 hover:bg-gray-50">
                  <div className="flex items-center mb-2">
                    ...
                    <p className="text-sm font-medium text-gray-600">Position</p>
                      ...
                      <p className="text-base text-gray-900 pl-6">{panel?.position ? `Row ${panel.position.row}, Column ${panel.position.column}` : ''}</p>
                </div>
                <div className="p-3 rounded-lg border border-gray-100 bg-white transition-all duration-300 hover:bg-gray-50">
                  <div className="flex items-center mb-2">
                ...
                <div className="p-3 rounded-lg border border-gray-100 bg-white transition-all duration-300 hover:bg-gray-50">
                  <div className="flex items-center mb-2">
                    ...
                    <p className="text-sm font-medium text-gray-600">Last Inspection</p>
                      ...
                      <p className="text-base text-gray-900 pl-6">{panel?.lastInspection ? formatDate(panel.lastInspection) : ''}</p>
                </div>
              </div>
                </>
              )}
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              <div className={`p-4 rounded-md animate-fadeIn transform transition-all duration-300 hover:scale-[1.02] ${
                mappedPriority === 'high' ? 'bg-red-50 border border-red-200' : 
                mappedPriority === 'medium' ? 'bg-yellow-50 border border-yellow-200' : 
                mappedPriority === 'low' ? 'bg-yellow-50 border border-yellow-100' : 'bg-green-50 border border-green-200'
              }`}>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-medium flex items-center">
                    <AlertTriangle className={`h-4 w-4 mr-2 ${
                      mappedPriority === 'high' ? 'text-red-600' : 
                      mappedPriority === 'medium' ? 'text-yellow-600' : 
                      mappedPriority === 'low' ? 'text-yellow-600' : 'text-green-600'
                    }`} />
                    <span className={
                      mappedPriority === 'high' ? 'text-red-800' :
                      mappedPriority === 'medium' ? 'text-yellow-900' :
                      mappedPriority === 'low' ? 'text-yellow-800' : 'text-green-800'
                    }>
                      {mappedPriority === 'none' ? 'No Fault Detected' : `${mappedPriority.charAt(0).toUpperCase() + mappedPriority.slice(1)} Priority Fault`}
                    </span>
                  </h3>
                </div>
                <p className={`text-sm ml-6 ${
                  mappedPriority === 'high' ? 'text-red-700' : 
                  mappedPriority === 'medium' ? 'text-yellow-800' : 
                  mappedPriority === 'low' ? 'text-yellow-800' : 'text-green-700'
                }`}>{faultDescription}</p>
              </div>

              <div className="mt-6 bg-gray-50 p-4 rounded-lg animate-fadeIn" style={{animationDelay: "0.2s"}}>
                <h3 className="font-medium text-gray-800 mb-2 flex items-center">
                  <Wrench className="h-4 w-4 text-blue-500 mr-2" />
                  Maintenance Suggestion
                </h3>
                <p className="text-sm text-gray-700 ml-6 border-l-2 border-blue-200 pl-3 py-2">{maintenanceSuggestion}</p>
              </div>
            </Card>

            <Card title="Inspection History" className="overflow-hidden transition-all duration-300 hover:shadow-md animate-slideInRight">
              {panel.inspectionHistory.length > 0 ? (
                <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                  {panel.inspectionHistory.map((record: any, index: number) => (
                    <div 
                      ...
                      style={{
                        borderLeftColor: record.faultLevel === 'high' ? '#ef4444' : 
                                        record.faultLevel === 'medium' ? '#f59e0b' : 
                                        record.faultLevel === 'low' ? '#fbbf24' : '#10b981',
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span 
                          className={`inline-block px-3 py-1 text-xs font-semibold rounded-full text-white ${
                            record.faultLevel === 'high' ? 'bg-red-600' : 
                            record.faultLevel === 'medium' ? 'bg-yellow-600' : 
                            record.faultLevel === 'low' ? 'bg-yellow-500' : 'bg-green-600'
                          }`}
                        >
                          {record.faultLevel}
                        </span>
                      </div>
                      <p className="text-sm text-gray-600 mt-2 ml-6">{record.description}</p>
                ...
                <div className="text-center py-8 bg-gray-50 rounded-lg animate-fadeIn">
                  ...
                  <p className="text-gray-600">No inspection history available.</p>
                </div>
              )}
            </Card>
          </div>
        ...
        <div className="mt-6">
          <Card className="overflow-hidden transition-all duration-300 hover:shadow-2xl animate-fadeIn bg-white border-0 relative">
            ...
            <div className="p-4">
              <h2 className="text-xl font-semibold mb-4">Inspection Priority History</h2>
                ...
                <ResponsiveContainer>
                  <LineChart
                    data={panel.inspectionHistory.map((record: any) => ({
                      ...
                      priority: (() => {
                        ...
                        if (desc.includes('dust') || desc.includes('bird') || desc.includes('snow')) return 2; // Medium
                        if (desc.includes('clean') || desc.includes('no fault') || desc.includes('healthy')) return 1; // Low/No fault
                  ...
                  >
                    ...
                    <XAxis 
                      ...
                      stroke="#4a5568"
                    >
                      <Label value="Inspection Date" offset={-10} position="insideBottom" />
                    </XAxis>
                    <YAxis
                      type="number"
                      domain={[1, 3]}
                      ...
                      tickFormatter={(val: number) =>
                        val === 1 ? 'No Fault/Low' : val === 2 ? 'Medium' : 'High'
                      }
                      allowDecimals={false}
                      width={100}
                    />
                    <Tooltip
                      labelFormatter={(date: string) => `Date: ${new Date(date).toLocaleString()}`}
                      formatter={(value: number) =>
                        value === 1 ? 'No Fault/Low' : value === 2 ? 'Medium' : 'High'
                      }
                    />
                    {/* Color areas for Y axis sections */}
                    <ReferenceArea y1={0.5} y2={1.5} fill="#bbf7d0" fillOpacity={0.2} />
                    <ReferenceArea y1={1.5} y2={2.5} fill="#fef9c3" fillOpacity={0.2} />
                    <ReferenceArea y1={2.5} y2={3.5} fill="#fecaca" fillOpacity={0.2} />
                    <Line
                      type="monotone"
                      dataKey="priority"
                      stroke="#6366f1"
